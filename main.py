...
# 모델, 스케일러, 인코더, 컬럼 순서 로드
model = tf.keras.models.load_model("fraud_model.h5")
scaler = joblib.load("scaler.pkl")
expected_columns = joblib.load("expected_columns.pkl")

label_encode_columns = ['gender', 'marital_status', 'address_change_ind',
...
def predict(data: InputData):
    try:
        ...
        df['claim_est_payout_group'] = pd.cut(df['claim_est_payout'], [0, 5000, 10000, 15000, float('inf')],
                                              labels=['0-5000', '5001-10000', '10001-15000', '15001+'], right=False)
        df['age_of_vehicle_group'] = pd.cut(df['age_of_vehicle'], [0, 5, 10, 15, float('inf')],
                                            labels=['0-5년', '6-10년', '11-15년', '16년 이상'], right=False)

        # 필요없는 원본 컬럼 제거
        drop_cols = ['age_of_driver', 'safty_rating', 'annual_income', 'vehicle_price',
                     'vehicle_weight', 'claim_est_payout', 'age_of_vehicle', 'liab_prct']
        ...
        # 원핫 인코딩
        df = pd.get_dummies(df, columns=['accident_site', 'channel', 'vehicle_category', 'vehicle_color'], drop_first=True)
